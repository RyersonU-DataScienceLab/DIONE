...
import metrics.CommonMetricTypes;
...
public class DataContext implements Serializable {
    ...
    private static boolean isLeafNode(String line) {
        return -1 < line.replace("\t", "").split(">")[0].indexOf(" ");
    }
    ...
    private void addSibling(Object key, Object obj) {
        ...
        if (current == null) {
            try {
                int i = Integer.parseInt(obj == null? "" : obj.toString());
                baseMap.put(key, i);
                return;
            } catch (Exception e) {
            }
            try {
                double d = Double.parseDouble(obj == null? "" : obj.toString());
                baseMap.put(key, d);
                return;
            } catch (Exception e) {
            }
    ...
    public Iterator<String> iterator() {
        ...
        while (ito.hasNext()) {
            Object o = ito.next();
            try {
                int id = Integer.parseInt(o == null? "" : o.toString());
                String s = MetricNamesMap.inverse(id);
                stringList.add(s);
            } catch (Exception e) {
                stringList.add(o == null? "" : o.toString());
            }
